...
export default function PortfolioClient() {
  ...
  return (
    <div className="bg-[var(--background)] text-[var(--foreground)] overflow-y-auto overflow-x-hidden py-22 sm:py-8 md:py-10 lg:py-12 px-4 sm:px-6 md:px-10 lg:px-[30px] relative custom-scrollbar" style={{ minHeight: '100vh', height: '100%', scrollbarWidth: 'thin', scrollbarColor: 'var(--nav-text) var(--background)' }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: isVisible ? 1 : 0, y: isVisible ? 0 : 20 }} transition={{ duration: 0.5 }} className="text-center mb-10">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4"><span className="text-[var(--primary)]">My</span> Portfolio</h1>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: isVisible ? 1 : 0, y: isVisible ? 0 : 20 }} transition={{ duration: 0.5, delay: 0.1 }} className="flex flex-wrap justify-center gap-2 sm:gap-4 mb-8">
        {categories.map((category, index) => (
          <button key={index} onClick={() => setSelectedCategory(category)} className={`px-4 py-2 rounded-full text-sm sm:text-base transition-all duration-300 ${selectedCategory === category ? 'bg-[var(--primary)] text-[var(--nav-text-hover)]' : 'bg-[var(--card-bg)] hover:bg-[var(--primary-hover)] hover:text-[var(--nav-text-hover)]'}`}>
            {category}
          </button>
        ))}
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
        {filteredProjects.map((project, index) => (
          <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: isVisible ? 1 : 0, y: isVisible ? 0 : 20 }} transition={{ duration: 0.5, delay: 0.1 + index * 0.1 }} className="bg-[var(--card-bg)] rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 hover:translate-y-[-5px] group">
            ...
            <div className="p-4 sm:p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {project.tags.map((tag, tagIndex) => (
                  <span key={tagIndex} className="text-xs px-2 py-1 rounded-full bg-[var(--primary)] bg-opacity-20 text-[var(--primary)]">{tag}</span>
                ))}
              </div>
            </div>
          </motion.div>
